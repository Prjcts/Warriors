...
class Warrior {
	...
	constructor(obj: Omit<Warrior, 'insert' | 'update'>) {
		...
		if (sum !== 10) {
			throw new ValidationError(`Wrong statistic number, the sum of them must be 10, but it is: ${sum}`);
		}
	...
	async insert(): Promise<string | undefined> {
		await pool.execute(
			'INSERT INTO `warrior`(`id`, `name`, `strength`, `defence`, `stamina`, `agility`, `wins`) VALUES(:id, :name, :strength, :defence, :stamina, :agility, :wins)',
			{
				id: this.id,
				/*
				 * Name is always unique - set in database as unique index
				 **/
				name: this.name,
	...
	static async isNameTaken(name: string): Promise<boolean> {
		const [result] = (await pool.execute(
			'SELECT * FROM `warrior` WHERE `name` = :name',
			{
				name,
			},
		)) as WarriorRecordResult;
